...
<Tabs items={['React Native', 'Flutter']}>
  ...
  <Tabs.Tab>
<Callout type="info" emoji="📱">
    A flutter sample app is coming soon  🚀.
</Callout>
    <Steps>
        ### Setup Screens
        Create a dummy screen for navigation testing. Make a new folder named `screens` inside the `lib` folder and create a file named `home_screen.dart` inside this folder.
        
        ```dart filename="home_screen.dart" copy
