...
Now that we have set up the Google Sign-In library, we can handle the sign-in process. We will create a new screen and name it `LoginScreen` and here we will create a function to handle the Google Sign-In process and call it when the user presses a button.
...
After a successful authentication, the `result` will contain the user's wallets information. We can use this information to personalize the user's experience in our app.
</Steps>

And with that we have successfully authenticated with Okto using Google Sign-In.
