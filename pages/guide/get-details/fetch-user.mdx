...
<Tabs items={['React Native', 'Flutter']}>
  ...
  <Tabs.Tab>
    <Steps>
      ### Fetch User Details
      First, let's create a new screen with the name `user_profile.dart`. Fetch the user details using [`getUserDetails`](/sdk-reference/flutter/getUserDetails) from Okto and store them in a state.
