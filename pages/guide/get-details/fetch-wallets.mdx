...
<Tabs items={['React Native', 'Flutter']}>
  ...
  <Tabs.Tab>
    <Steps>
      ### Fetch Wallets
      First, let's fetch the wallets information of the user using the [`getWallets`](/sdk-reference/getWallets) method. We will continue developing in the `user_profile.dart`.
