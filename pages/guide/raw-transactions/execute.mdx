...
<Steps>
    ...
    ### Send a transaction
    Now we can use the `api` instance to send a transaction to the blockchain. Create a new screen and name it `send-raw-transaction.tsx`, here we will cover both EVM and Non EVM transactions.
    <Tabs items={["EVM", "Non EVM"]}>
        <Tabs.Tab>
